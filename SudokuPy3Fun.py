...
def myBootstrap(M,fillMatrix):
    ...
    if step==0:
        ...
        for stepNum in range(0,step+1):
            ...
            if (stepNum<step):
                listTemp = [j for j in range(stepNum*int(1E6), (stepNum+1)*int(1E6))]
            else:
                listTemp = [j for j in range(step*(1E6),total)]
...
def makeCase(M, fillMatrix, list_of_dualBlank, numDB):
    ...
    for myNum in range(0,pow(2,numDB)):
        ...
        for k in range(0,numDB):
            caseArray[0,myNum][0,k] = int(choice_of_dualBlank[k,int(posArray[myNum,k])])
...
def myExhaustFill(M, fillMatrix):
    ...
    for num in range(0,81):
        if M01[num//9,num%9]!=0:
            myDoneList.append(num)
    ...
    while (number_of_blank>0):
        ...
        for num in range(0, 81):
            no_of_zero = 0
            for jj in range(0, 9):
                if fillMatrix01[num, jj]==0:
                    no_of_zero += 1
            if (no_of_zero == 8) and (num not in myDoneList):
                myDoneList.append(num)
                number_of_blank =number_of_blank-1
                M01[num//9, num%9] = int(sum(fillMatrix01[num,:]))
                fillMatrix01 = fillBlank(M01)
                fPoint = True
                break
        ...
        for i in reference:
            for rowNum in range(0,9):
                ...
                if (appearance==1)and(M01[rowNum,fillpos]==0):
                    ...
                    fillMatrix01 = fillBlank(M01)
                    number_of_blank = number_of_blank-1
                    myDoneList.append(9*rowNum+fillpos)
                    bPoint = True
                    break
            ...
            for colNum in range(0,9):
                ...
                if (appearance==1)and(M01[fillpos,colNum]==0):
                    ...
                    fillMatrix01 = fillBlank(M01)
                    number_of_blank = number_of_blank-1
                    myDoneList.append(9*fillpos+colNum)
                    bPoint = True
                    break
...
M1008 = np.array([[8, 7, 1, 3, 4, 5, 0, 0, 9],
                  [0, 4, 0, 0, 0, 0, 1, 0, 8],
                  [0, 0, 0, 0, 9, 0, 3, 0, 4],
                  [0, 3, 0, 4, 7, 9, 5, 8, 0],
                  [0, 9, 8, 0, 0, 2, 4, 0, 7],
                  ...
                  [0, 0, 0, 6, 8, 0, 0, 4, 3],
                  [6, 0, 0, 0, 0, 0, 0, 0, 0]])
fM1008 = fillBlank(M1008)
##t1008, ft1008, v1008 = myExhaustFill(M1008, fM1008)
        ...
        if valid05==False:
            ...
            if (numDB==0):
                ...
                for i in range(0,pow(2,numDB)):
                    ...
                    # updated codelines:
                    if (valid01==False) and (i!=pow(2,numDB)-1):
                        continue
                    if (valid01==False) and (i==pow(2,numDB)-1):
                        return M01, fillMatrix01, False
                    else:
                        ...
                        if (valid04==False) and (i==pow(2,numDB)-1):
                            return  M01, fillMatrix01, False
...
def makeCaseNDB(M, fMatrix):
    ...
    start = 0
    for i in range(0, 81):
        if (countVec[1][i]!=0) and (countVec[1][i]!=1):
            posVec.append(int(countVec[0][i]))
            start = start+1
        if start==5: #layer is bounded by 5
            break
    layer = len(posVec) #layer=5

    choice_of_blanks = np.empty([1,layer],dtype=object)
    for Num in range(0,layer):
    ...
    posArray = np.zeros([caseTotal, layer])
    ...
    for myNum in range(0,caseTotal):
        caseArray[0,myNum] = np.zeros([1,layer])
        for j in range(0,layer):
...
def NDBsolveEngine(M, fillMatrix, valid, total):
    ...
    if valid01==False:
        ...
        print("NDBsolveEngine: start bootstrapping")
        ...
        if valid03==False:
            ...
            for i in range(0,caseTotal):
                ...
                valid01 = checkConsistent(M01, fillMatrix01)
                total01 = countCombination(fillMatrix01)
                if (valid01==False) and (i!=caseTotal-1):
                    continue
                elif (valid01==False) and (i==caseTotal-1):
                    return M01, fillMatrix01, False
                else:
                    M04, fillMatrix04, valid04 = NDBsolveEngine(M01, fillMatrix01, valid01, total01)
                    if valid04==True:
                        return M04, fillMatrix04, True
                    if (valid04==False) and (i==caseTotal-1):
                        return M01, fillMatrix01, False
