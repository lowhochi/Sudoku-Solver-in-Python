...
# solve the sudoku by exhausting all possible outcomes.
def myRawInput():
    A = np.zeros([4,4])
    print "Enter the first row:",
    stringFirstRow = raw_input().split(' ')
    print "Enter the second row:",
    stringSecondRow = raw_input().split(' ')
    print "Enter the third row:",
    stringThirdRow = raw_input().split(' ')
    print "Enter the fourth row:",
    stringFourthRow = raw_input().split(' ')

    myFirstRow = [int(num) for num in stringFirstRow]
    mySecondRow = [int(num) for num in stringSecondRow]
    myThirdRow = [int(num) for num in stringThirdRow]
    myFourthRow = [int(num) for num in stringFourthRow]
    for j in range(0,4):
        A[0,j]=myFirstRow[j]
        A[1,j]=mySecondRow[j]
        A[2,j]=myThirdRow[j]
        ...
        for j in range(0,9):
            ...
            if M[i,j]!=0:
                ...
                for k in range(0,9):
                    ...
                    if k!=j:
                        fillMatrix[9*i+k,M[i,j]-1]=0
                    if k!=i:
                        fillMatrix[9*k+j,M[i,j]-1]=0
                # block-check
                for m in myBlock:
                    if (m!=(9*i+j)):
                        fillMatrix[m,M[i,j]-1]=0
    return fillMatrix

# myArray = a numpy array of length = 81
...
def myBootstrap(M,fillMatrix):
    ...
    print "\nRunning: myBootstrap(M,fillMatrix)"
    ...
    for rowNum in range(0,81):
        ...
        for j in range(0,9):
            if fillMatrix[rowNum,j]!=0:
                myCount+=1
        tempNum = int(tempNum/myCount)
        divisor.append(tempNum)

    position = np.zeros([1,81])
    step = int(total/long(1E6))
    ...
    if step==0:
        for Num in range(0,total):
            if (Num%1000==0):
                print "Num =", str(Num)
            for k in range(0,81):
                position[0,k] = int(Num/divisor[k])
...
def makeCase(M, fillMatrix,list_of_dualBlank, numDB):
    ...
    for myNum in range(0,pow(2,numDB)):
        ...
        for j in range(0,numDB):
            posArray[myNum,j] = int(temp/pow(2,numDB-1-j))
...
# similar to the solve engine but without self-recursion. 
...
def PreSolveEngine(M,fillMatrix, valid):
    ...
    print "number of blank =", no_of_blank
    if valid==False:
        return M, fillMatrix, False
    elif no_of_blank<10:
        print "number of blanks<10"
        print M
        ...
        if numDB==0:
            ...
            for i in range(0,pow(2,numDB)):
                ...
                for j in range(0,numDB):
                    Num1 = int(list_of_dualBlank[j]/9)
...
def myExhaustFill(M, fillMatrix):
    ...
    fillMatrix01 = np.zeros([81,9])
    fillMatrix01[:][:] = fillMatrix[:][:]
    print "\nRunning: myExhaustFill(M, fillMatrix)"
    number_of_blank = countBlank(M)
    valid = checkConsistent(M, fillMatrix)
    if valid==False:
        print "Inconsistent input"
        ...
        for i in reference:
            ...
            for myNum in [0,3,6,27,30,33,54,57,60]:
                ...
                if (appearance==1)and (M01[int(fillpos/9), int(fillpos%9)]==0):
                    M01[int(fillpos/9), int(fillpos%9)]=i
                    fillMatrix01 = fillBlank(M01)
                    number_of_blank = number_of_blank-1
                    bPoint = True
                    break
    ...
    print "Number of blanks left =", str(blankLeft)
    return M01, fillMatrix01, valid01
...
def mySolveEngine(M, fillMatrix, valid, total):
    ...
    fillMatrix01 = np.zeros([81,9])
    fillMatrix01[:][:] = fillMatrix[:][:]
    print "\nRunning: mySolveEngine(M, fillMatrix, valid, total)"
    # start_time = time.time()
    if valid==False:
        print "Inconsistent input"
        return M01, fillMatrix01, False
        ...
        if valid05==False:
            ...
            if (numDB==0):
                ...
                Mtup = tuple(M01)
                print "Make Case. Number of DB(<=15) :", str(min(numDB,15)) 
                ...
                for i in range(0,pow(2,numDB)):
                    ...
                    if i==(pow(2,numDB)-1):
                        ...
                        # Recursion
                        print "Recursion in. Current puzzle is:"
                        print M01
...
def makeCaseNDB(M, fMatrix):
    ...
    for myNum in range(0,caseTotal):
        ...
        divisor = caseTotal
        for j in range(0,5):
            divisor = divisor/len(choice_of_blanks[0][j])
            posArray[myNum][j] = int(tempNum2/divisor)
...
# another solving engine

# Vec1: an array of 1xN1 elements
# Vec2: an array of 1xN2 elements
def myObjectVec(Vec1, Vec2):
    N1 = len(Vec1)
    N2 = len(Vec2)
    myObjVec = np.empty([1,N1*N2], dtype=object)
    myObjVecTwo = np.empty([1,N1*N2], dtype=object) 
    myCount = 0
    for i in range(0,N1):
        for j in range(0,N2):
           myObjVec[0,myCount] = np.array([Vec1[i],Vec2[j]])
           myCount = myCount + 1

    myCount2 = 0
    for i in range(0,N1*N2):
        myNumber = myObjVec[0,i][0]*myObjVec[0,i][1]
        if (myNumber!=0):
            myObjVec[0,myCount2] = myObjVec[0,i] # Replacement
            myCount2 = myCount2+1
    myObjVecTwo = np.empty([1,myCount2], dtype=object) 
    for j in range(0,myCount2):
        myObjVecTwo[0,j] = myObjVec[0,j]
    return myObjVecTwo

# objVec1: an 1xN1 object array of elements = 1xN-array
# testVec2: an array of 1xN2 elements
def myObjectVecB(objVec, ayVec, N):
    myCount =0
    N1 = objVec.shape[1]
    N2 = len(ayVec)
    myObjVec = np.empty([1,N1*N2], dtype=object) 
    for i in range(0,N1):
        for j in range(0,N2):
            myObjVec[0,myCount] = np.append(objVec[0,i],ayVec[j])
            myCount = myCount + 1
    ...
    for i in range(0,N1*N2):
        ...
        for j in range(0,N+1):
            myNumber = myNumber*myObjVec[0,i][j]
        if (myNumber!=0):
            myObjVec[0,myCount2] = myObjVec[0,i]
            myCount2 = myCount2+1
    myObjVecTwo = np.empty([1,myCount2], dtype=object) 
    for j in range(0,myCount2):
        myObjVecTwo[0,j] = myObjVec[0,j]
...
def solveEngine2(Atwo,fillMatrix2):
    totalNumber2 = countCombination(fillMatrix2)
    tempProductVec = np.empty([1,1], dtype=object) 
    tempProductVec= sudoku3Fun.myObjectVec(fillMatrix2[0,:], fillMatrix2[1,:])
    for i in range(1,80):
        tempProductVec = sudoku3Fun.myObjectVecB(tempProductVec,fillMatrix2[i+1,:], i+1)
    allOutcome = tempProductVec
    tempOutcome = np.empty([1,totalNumber2], dtype=object)
    myCount = 0
    for j in range(0,totalNumber2):
        tempArray = allOutcome[0,j]
        rowValid = sudoku3Fun.myRowCheck(tempArray)
        columnValid = sudoku3Fun.myColumnCheck(tempArray)
        blockValid = sudoku3Fun.myBlockCheck(tempArray)
        if (rowValid==False)or(columnValid==False)or(blockValid==False):
            continue
        tempOutcome[0,myCount] = tempArray
        myCount = myCount+1  
    possibleOutcome = np.empty([1,myCount], dtype=object)

    for j in range(0,myCount):
        possibleOutcome[0,j] = tempOutcome[0,j]
